...
GraphRAG Pipeline (7 layers):
  [1] Hybrid Retrieval: BM25 + GIST (lexical + semantic)
  [2] RRF Fusion → Hybrid Seeds (Fibonacci cascade: 169 → 144 chunks)
  [3] Graph Expansion from seeds (Node2Vec - placeholder)
  [4] RRF Fusion (Hybrid + Graph → k² = 169 chunks)
...
class BaseGISTRetriever(PGVectorRetriever):
    ...
    def search(
        ...
        graph_expansion_limit = retrieval_limit * 2  # Double for graph expansion: 338
        
        print(f"GraphRAG Pipeline: BM25+GIST {retrieval_limit} -> RRF {hybrid_seeds} hybrid seeds -> Graph {graph_expansion_limit} -> RRF fusion -> Reconstruct documents -> Late Interaction (score all) -> Select {top_k} final")
