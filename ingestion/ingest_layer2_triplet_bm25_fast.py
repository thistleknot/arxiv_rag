"""
LAYER 2: TRIPLET BM25 SPARSE VECTORS - FAST BERT IMPLEMENTATION

Fixes vocabulary mismatch by using BERT tokenization consistently.
Optimized to process 161K chunks in reasonable time (~5-10 minutes).

Key optimization: Direct TF-IDF computation instead of per-document BM25 scoring.
"""
...
def load_data():
    ...
    with open('checkpoints/chunks.msgpack', 'rb') as f:
        chunks_data = msgpack.unpack(f, strict_map_key=False)
        chunks = {c['id']: c for c in chunks_data}
    print(f"  ✓ Loaded {len(chunks):,} chunks")
    
    with open('checkpoints/chunk_to_triplets.msgpack', 'rb') as f:
        chunk_to_triplets = msgpack.unpack(f, strict_map_key=False)
    print(f"  ✓ Loaded {len(chunk_to_triplets):,} chunk→triplet mappings")
    
    with open('checkpoints/triplet_bm25_index.msgpack', 'rb') as f:
        triplet_data = msgpack.unpack(f, strict_map_key=False)
        triplet_texts = {tid: text for tid, text in zip(
            triplet_data['triplet_ids'],
            triplet_data['triplet_texts']
        )}
...
def aggregate_triplets_to_chunks(chunks, chunk_to_triplets, triplet_texts):
    """Aggregate triplet texts to chunk level"""
    print("\nAggregating triplets to chunk level...")
    
    chunk_aggregated = []
    skipped = 0
    
    for chunk_id in tqdm(sorted(chunks.keys()), desc="  Processing"):
        triplet_ids = chunk_to_triplets.get(chunk_id, [])
        ...
        combined_text = ' '.join([
            triplet_texts[tid] for tid in triplet_ids 
            if tid in triplet_texts
        ])
...
def main():
    ...
    # Aggregate triplets
    chunk_aggregated = aggregate_triplets_to_chunks(chunks, chunk_to_triplets, triplet_texts)
