"""
Ingest Layer 2 BIO-Triplet BM25 as sparsevec(16000) with signed feature hashing.

Input:  checkpoints/bio_triplets_full_corpus.msgpack
        format: [{doc_id, paper_id, section_idx, chunk_idx,
                  triplets: [{subject, predicate, object, arity, relation_type},...]}]

Output: layer2_triplet_bm25.triplet_bm25_vector = sparsevec(16000)
Stats:  bm25_l2_stats (n_docs, total_tokens)
        bm25_l2_term_df (bucket, doc_freq)

BM25 terms are derived by tokenising the concatenated span text of each triplet:
    f"{subject} {predicate} {object}".lower().split()
...
Hashing: mmh3 signed trick (Weinberger et al. 2009)
    ...
    Identical to retrieval/pgvector_retriever.py hash_token() -- query/ingest aligned.
"""
...
MSGPACK_PATH = Path("checkpoints/bio_triplets_full_corpus.msgpack")
...
# ---------------------------------------------------------------------------
# Term extraction from BIO triplets
# ---------------------------------------------------------------------------

def extract_terms(triplets: List[dict]) -> List[str]:
    """
    Tokenise subject + predicate + object spans for each triplet.
    Returns a flat list of lowercase tokens (duplicates allowed for TF).
    """
    terms: List[str] = []
    for t in triplets:
        ...
        obj  = t.get("object") or ""
        span = f"{subj} {pred} {obj}".strip()
        if span:
            terms.extend(span.lower().split())
    return terms
...
def load_bio_triplets() -> List[Tuple[str, List[str]]]:
    """
    Load bio_triplets_full_corpus.msgpack.
    Returns list of (chunk_id, [term, ...]) with duplicates for TF.
    """
    path = MSGPACK_PATH
    ...
    for record in data:
        ...
        if chunk_id is None:
            skipped += 1
            continue
        terms = extract_terms(record.get("triplets", []))
        if terms:
...
def ingest(all_docs: List[Tuple[str, List[str]]]) -> None:
    ...
    # ---- Pass 1: corpus stats ----------------------------------------------
    print("\nPass 1: computing corpus stats from BIO triplet terms...")
    ...
    print(
        f"  Stats: n_docs={n_docs:,}, total_tokens={total_tokens:,}, "
...
def verify() -> None:
    ...
    cur.execute(
        "SELECT COUNT(*) FROM layer2_triplet_bm25 WHERE triplet_bm25_vector IS NOT NULL"
    )
...
def main() -> None:
    print("=" * 60)
    print("LAYER 2 BIO-TRIPLET BM25 INGESTION  sparsevec(16000) signed-hash")
    print("=" * 60)

    if not MSGPACK_PATH.exists():
        print(f"ERROR: {MSGPACK_PATH} not found — run apply_bio_corpus.py first")
        raise SystemExit(1)

    all_docs = load_bio_triplets()
    if not all_docs:
        print("ERROR: no documents loaded — check triplet output format")
        raise SystemExit(1)

    ingest(all_docs)
    verify()

    print("\nLAYER 2 BIO-TRIPLET BM25 INGESTION COMPLETE")
